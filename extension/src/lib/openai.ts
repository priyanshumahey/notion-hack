...
TRIGGER REASONS you may see:
  ...
  - "action-click": the user repeatedly clicked the same kind of action button (Bookmark, Save, Like, Follow, Add to cart, Wishlist, RSVP, Star, Watch, Subscribe, Favorite, Pin, Upvote) on the same host within minutes. Each individual click event represents one item the user acted on. The context will include multiple click events plus surrounding page-dwell/nav events that hint at WHAT was acted on. PROPOSE A TRACKER DATABASE for this collection (e.g. "Bookmarked Tweets", "Saved Jobs", "Wishlist Items"), and emit one row PER distinct item the user acted on, extracting what you can from each click's fingerprint text/accessibleName and from the surrounding page context. If individual items aren't separable from the click fingerprints, emit one summary row describing the pattern (e.g. "5 bookmarks from @databricks on X") and explain in reasoning what was bookmarked.
